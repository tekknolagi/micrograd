...
import struct
import time
from micrograd.nn import MLP, Value
from micrograd.compiler import make_extension
...
for k in range(TRAINING_ROUNDS):
    ...
    outputs = [nn(image.pixels) for image in inputs]
    # losses = [(1 + -yi * scorei).relu() for yi, scorei in zip(labels, outputs)]
